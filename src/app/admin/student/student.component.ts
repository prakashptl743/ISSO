...
setFiletr() {
  ...
  this.studentService.getCustomersLarge( this.eventValue,this.schoolvalue).then(customers => {
    ...
    response => {
      if(response!=="") {
        ...
        if (response[0].eleven_boys > 0 && this.selectedAge == '11' ) {
          ...
          this.isMessageshow = true;
        } else if (response[0].sixteen_boys > 0 && this.selectedAge == '16') {
          this.studentCapacity = response[0].sixteen_boys;
          this.isMessageshow = true;
        } else if (response[0].sixteen_girls > 0 && this.selectedAge == '16') {
...
loadStudentDataForSubGame() {
  ...
  this.studentDataArray.push(
    this.eventIdForStudent, this.gameId, this.selectedAge,this.selectedGender,this.newSchoolId
  );
  const formData = new FormData();
  formData.append('studentData', JSON.stringify(this.studentDataArray));
  this.studentEnrollmentService.loadStudentData(formData).subscribe(
        ...
        if(this.studentListArray.length > 0 ) {
         ...
         // this.isStudentListShow = true;
          if(this.showSubGameList) {
            ...
            console.log('HELLOO++++>'+JSON.stringify(this.newSubGameCapacity));
            let  capaCityCount = 0;
            console.log('this.mapSubGameTeam.length---->'+this.mapSubGameTeam.length);
            if (this.mapSubGameTeam.length > 0 ) {
              for(let i=0;i <=this.mapSubGameTeam.length - 1;i++) {
                console.log('I value===>'+i);
        ...
        if(response!=="") {
          ...
          if(this.showSubGameList) {
            for (let i=0; i <= this.schoolServiceData.length - 1;i++) {
               this.checkSubGameCapacity.push(this.schoolServiceData[i]['subgameId']);
            }
            var commaSeperatedString = this.checkSubGameCapacity.toString(); 
            this.testArray = commaSeperatedString.split(',');
            for(let i =0; i<=this.testArray.length-1;i++) {
            ...
            console.log('DATA===>'+JSON.stringify(this.newSubGameCapacity))
            if (this.mapSubGameTeam.length > 0 ) {
              for(let i=0;i <=this.mapSubGameTeam.length - 1;i++) {
                console.log('I value===>'+i);
                let  capaCityCount = 0;
...
loadSubGameChange(subgame) {
 ...
 const subGameIDForCapcity =  this.subGameId;
 const capaCityCount = this.newSubGameCapacity.filter((obj) => obj.subGameId === subGameIDForCapcity).length;
...
//  this.addClass = false;
 ...
 // this.gameId = gameId;
  ...
  this.studentEnrollmentService.getSubGameData(this.gameId, this.selectedAge,this.genderForSubGame,'admin').subscribe(
    response => {
      if(response!=="") {
        this.subGameList = response;
        this.subGameoptions = [];
