...
loadRootGame(){
  this.gameService.getRootGameList().subscribe(response => {
    if(response!=="") {
      ...
      if(this.subgameId === '') { 
        ...
        res => {
            if (res.status === 'error') {
              ...
              this.messageService.add({key: 'custom', severity:'success', summary: 'New Sub Game Added Successfully'});
...
deleteSubGameData(event: Event, subGameData: SubGame) {
    ...
    this.confirmation.confirm({
      ...
      accept: () => { this.deleteGame(subGameData); },
    });
}
