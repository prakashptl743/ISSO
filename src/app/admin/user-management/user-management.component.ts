 ...
 if (this.mapMenuArray.length > 0 ) {
   for (let i=0;i< this.mapMenuArray.length;i++) {
     if (this.menuVal === this.mapMenuArray[i].menu_id) {
        ...
        this.messageService.add({key: 'custom',  severity:'error', summary: 'This menu already exists!'});
     } else {
       console.log('im else');
...
deleteUser(userId) {
  this.userService.deleteUser(userId).subscribe(
   res => {
         this.messageService.add({key: 'custom', severity:'success', summary: 'User Data Deleted Successfully'});
     this.getUsers();
   },
  ...
  this.userService.changeMenuStatus(menuId, formData).subscribe(
    res => {
        if (res.status === 'success') { 
          this.messageService.add({key: 'custom', severity:'success', summary: 'Menu Data Updated Successfully'});
       } 
      else {
         this.messageService.add({key: 'custom', severity:'success', summary: 'Menu Data Updated Successfully'});
        }
...
changeUserStatus(menuId,val) {
  ...
  this.userService.changeUserStatus(menuId, formData).subscribe(
    res => {
        if (res.status === 'success') { 
          this.messageService.add({key: 'custom', severity:'success', summary: 'User Data Updated Successfully'});
       } 
      else {
         this.messageService.add({key: 'custom', severity:'success', summary: 'User Data Updated Successfully'});
        }
