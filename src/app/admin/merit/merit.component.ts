 ...
 toDataURL(url, callback) {
        ...
        if(response!=="") {
          ...
          if(this.gameList.length > 0 ) {
            ...
            for(let i=0;i<this.gameIdList.length;i++) {
              this.myObjArray.push({gameId: Number(this.gameIdList[i]), game_name: this.gameNameList[i], gameType: this.gameTypeList[i] });
            } 
  ...
  loadGenderChange(gender) {
    ...
    this.selectedStudent= '';
     ...
     this.meritService.loadSubgameByGame(this.gameId,this.selectedAge,this.genderVal,this.isPrintMerit).subscribe(
      response => {
        if(response!=="") {
          ...
          this.messageService.add({key: 'custom', severity:'error', summary: 'Sub Game Not Found'});
          ...
          response => {
            if(response!=="") {
               ...
               //   this.schoolList = response;
                  ...
                  console.log('Im both');
                    if(this.schoolList[0].subgameType == 'Individual' || this.schoolList[0] == 'Individual'){
                    ...
                    if(this.schoolList[0].subgameId == '' && this.schoolList[0].subgameId == '') {
  ...
  this.meritService.showAlreadyMeritData(this.eventValue,this.gameId,this.selectedAge,this.genderVal,this.subGameId).subscribe(
    res => {
      console.log(res);
      if(res!=="") {
  ...
  printIndividualCertificate(studentData:any) {
    ...
    this.printIndividualMeritArray.push(
      studentData.schoolId,studentData.eventId,studentData.gameId,studentData.id,studentData.ageRange,studentData.gender,studentData.sId
     ); 
    const formData = new FormData();
    formData.append('meritData', JSON.stringify(this.printIndividualMeritArray));
    this.meritService.loadIndividualCertificate(formData).subscribe(response => {
      if(response!=="") {
        this.certificateData = response;
        this.downloadCertificatePdf(studentData.studentName,studentData.rank);
  ...
  printCertificate(schoolId: any, schoolName: any,rank: any) {
    ...
    this.meritService.loadCertificateData(this.selectedYearVal,1, this.eventValue,this.gameId,schoolId).subscribe(
      response => {
        if(response!=="") {
          this.certificateData = response;
 ...
 // let str4 : any;
  ...
  for(let i=0; i< this.certificateData.length; i++) {
    ...
    str6 =  {
              ...
              table: {
                widths: ['*'],
                body: [ 
                  ['VENUE : ' + this.certificateData[i].location +'\n'+ this.datepipe.transform(this.certificateData[i].startDate, 'd MMM, y') +' - '+ this.datepipe.transform(this.certificateData[i].endDate, 'd MMM, y')],
                ]
              }
            };
              ...
              columns: [
                {
                  ...
                  table: {
                    ...
                    body: [
                      ...
                      ['FATHER’S/MOTHER’S NAME: ',+ this.certificateData[i].fatherName],
                      ['DATE OF BIRTH: ', this.datepipe.transform(this.certificateData[i].dateOfBirth, 'd MMM, y')],
                      ['CLASS: ', this.certificateData[i].standardClass+'                                                   ' +'ISSO/'+this.certificateData[i].sId],
    ...
    //         ]
    //       };
    if(this.certificateData[i].gameType == 'Team' || this.certificateData[i].gameType == 'Individual') {
        ...
        pageBreak: "after",
         text: [ 
           'HAS PARTICIPATED IN 2ND ISSO NATIONAL GAMES 2019-20 IN',
            ...
            { text: 'SUBGAME ' + this.certificateData[i].subGameName, style: 'boldText' },
            { text: ' UNDER', style: 'boldText' },
            { text: this.certificateData[i].ageRange + genderval, style: 'boldText' },
           ' HELD AT ',
  ...
  loadSchoolChange(schoolData) {
    ...
    if (this.subGameId != '' && this.subGameType == 'Individual' || this.gameType =='Individual') {
      ...
      this.isDataAvailble = false;
      this.addMeritDataArray.push(
        this.eventValue,this.gameId,this.subGameId,this.subGameType,this.selectedAge,this.genderVal,this.schoolId
      );
          ...
          if(response!=="") {
            ...
            if(this.studentList.length > 0 ) {
              ...
              this.studentList.forEach(element => {
                const studentIdAndName = element.sId +','+ element.studentName;
                this.studentOptions.push({
            ...
            this.isShowStudent = false;
            this.messageService.add({key: 'custom', severity:'error', summary: 'Student not found'});
           }
          } else {
            console.log('Data is blannk from service')
          }
...
checkNOsugameMerit(){
  ...
  this.meritService.checkNOsugameMerit(this.eventValue,this.gameId,this.subGameId,this.selectedAge,this.genderVal, this.schoolId).subscribe(
    response => {
      console.log('test'+response);
      
    if(response !=="true") {
      this.messageService.add({key: 'custom', severity:'error', summary: 'No merit found for this school'});
      this.isShowStudent = false;
      this.selectedStudent ='';
      this.rankReadble = false;
    } else {
...
checkNoMerti() {
  this.meritService.loadStaffReport(this.yearvalue,0, this.eventValue,this.gameId, this.schoolId).subscribe(
    response => {
    if(response=="") {
      this.messageService.add({key: 'custom', severity:'error', summary: 'No merit found for this game'});
      this.isShowStudent = false;
      this.selectedStudent ='';
      this.rankReadble = false;
    } else {
...
addMeritData() {
  ...
  if (this.showAlreadyMeritData.length > 0) {
     for (let i=0;i< this.showAlreadyMeritData.length;i++) {
      if(this.subGameType == 'Individual') {
        if (this.eventValue === this.showAlreadyMeritData[i].eventId && this.gameId === this.showAlreadyMeritData[i].gameId && this.subGameId === this.showAlreadyMeritData[i].subgameId  && this.selectedAge === this.showAlreadyMeritData[i].ageRange && this.genderVal === this.showAlreadyMeritData[i].gender && this.schoolId === this.showAlreadyMeritData[i].schoolId && this.rankValue === this.showAlreadyMeritData[i].rank &&  this.studentId === this.showAlreadyMeritData[i].studentID) {
          this.isAlreadyAddedMerit = true;
        }
      }
      if(this.subGameType == 'Team') {
        if (this.eventValue === this.showAlreadyMeritData[i].eventId && this.gameId === this.showAlreadyMeritData[i].gameId && this.subGameId === this.showAlreadyMeritData[i].subgameId  && this.selectedAge === this.showAlreadyMeritData[i].ageRange && this.genderVal === this.showAlreadyMeritData[i].gender && this.schoolId === this.showAlreadyMeritData[i].schoolId && this.rankValue === this.showAlreadyMeritData[i].rank) {
         this.isAlreadyAddedMerit = true;
        }
      }
      if(this.gameType == 'Team') {
        if (this.eventValue === this.showAlreadyMeritData[i].eventId && this.gameId === this.showAlreadyMeritData[i].gameId && this.selectedAge === this.showAlreadyMeritData[i].ageRange && this.genderVal === this.showAlreadyMeritData[i].gender && this.schoolId === this.showAlreadyMeritData[i].schoolId) {
          this.isAlreadyAddedMerit = true;
        }
      }
    }
  ...
  if (this.teamMeritArray.length > 0) {
    for (let i=0;i< this.teamMeritArray.length;i++) {
      if (this.eventValue === this.teamMeritArray[i].eventId && this.gameId === this.teamMeritArray[i].gameId && this.selectedAge === this.teamMeritArray[i].agerange && this.genderVal === this.teamMeritArray[i].genderId && this.schoolId === this.teamMeritArray[i].schoolId) {
  ...
  if (this.individualMeritArray.length > 0) {
    for (let i=0;i< this.individualMeritArray.length;i++) {
      if (this.eventValue === this.individualMeritArray[i].eventId && this.gameId === this.individualMeritArray[i].gameId && this.subGameId === this.individualMeritArray[i].subGameId  && this.selectedAge === this.individualMeritArray[i].agerange && this.genderVal === this.individualMeritArray[i].genderId && this.schoolId === this.individualMeritArray[i].schoolId && this.rankValue === this.individualMeritArray[i].rank &&  this.studentId === this.individualMeritArray[i].studentId) {
        this.isDuplicate = true;
      }
    }
  }
  ...
  if (this.teamSubGameArray.length > 0) {
    for (let i=0;i< this.teamSubGameArray.length;i++) {
      if (this.eventValue === this.teamSubGameArray[i].eventId && this.gameId === this.teamSubGameArray[i].gameId && this.subGameId === this.teamSubGameArray[i].subGameId  && this.selectedAge === this.teamSubGameArray[i].agerange && this.genderVal === this.teamSubGameArray[i].genderId && this.schoolId === this.teamSubGameArray[i].schoolId && this.rankValue === this.teamSubGameArray[i].rank) {
        this.isDuplicate = true;
      }
    }
  }
  ...
  if (this.isDuplicate) {
    this.messageService.add({key: 'custom',  severity:'error', summary: 'This Record is already exists!'});
  }
  if (this.isAlreadyAddedMerit) {
    this.messageService.add({key: 'custom',  severity:'error', summary: 'This Record is already exists in database!'});
  }
   ...
   if((this.subGameType == 'Individual' || this.gameType == 'Individual') && !this.isDuplicate && !this.isAlreadyAddedMerit ) {
   ...
   //   this.isMeritDataShow = false;
    ...
    } if(this.subGameType == 'Team' && !this.isDuplicate && !this.isAlreadyAddedMerit) {
      this.teamSubGame = true;
    ...
    } if(this.gameType == 'Team' && !this.isDuplicate && !this.isAlreadyAddedMerit)  {   
...
saveMeritData() {
    ...
    formData.append('individualMeritData', JSON.stringify(this.saveIndividualMeritArray));
    formData.append('teamMeritData', JSON.stringify(this.saveTeamMeritArray));
    formData.append('teamSubGameArray', JSON.stringify(this.saveTeamSubGameMeritArray));
