   ...
   constructor( private issoUtilService: IssoUtilService, private paymentInvoiceService: PaymentInvoiceService,private messageService: MessageService, private meritService: ReportMeritService) { }
    ...
    addPaymentForStudent(e,studentId,amount) {
      if(e.target.checked) {
       ...
       let removeIndex = this.mapStudentPaymentData.findIndex(itm => itm.studentId===studentId);
       if(removeIndex !== -1)
         this.mapStudentPaymentData.splice(removeIndex,1);
      }
      ...
      response => {
        if(response!=="") {
            ...
            if (this.schoolType == 'Yes') { 
              this.schoolPayment = this.reportDataLength * ( Number(response[0].price) * 1);
              this.totalDues = this.schoolPayment;
            } else {
              this.schoolPayment = this.reportDataLength * ( Number(response[0].price) * 2);
              this.totalDues = this.schoolPayment;
            }
            let isAffilated = 0;
            let notAffilated = 0;
            for (let i=0;i < this.reportData.length;i++) { 
               if (this.reportData[i].isPaid === 'Yes') {
                  ...
                  if (this.schoolType == 'Yes') { 
                    ...
                    this.totalDues = this.schoolPayment - (isAffilated * Number(response[0].price) * 1) ;
                  } else {
                    notAffilated = notAffilated+1;
                    console.log('im paid else-->'+notAffilated)
                    this.totalDues = this.schoolPayment - (notAffilated * Number(response[0].price) * 2) ;  
                  }
    ...
    this.paymentInvoiceService.saveStudentAttendance(formData).subscribe(
      response =>  {
        ...
        if (response.status === 'error') {
          ...
          this.totalPaidTillNow = this.schoolPayment - this.totalDues;
          this.messageService.add({key: 'custom', severity:'success', summary: 'Payment Data updated successfully'});
