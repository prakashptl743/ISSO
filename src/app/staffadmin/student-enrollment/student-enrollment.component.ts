import { ChangeDetectorRef, Component, OnInit, Renderer, Renderer2, ViewChild } from '@angular/core';
...
export class StudentEnrollmentComponent implements OnInit {
  ...
  categories1: any[] = [{name: 'Accounting', key: 'A'}, {name: 'sds', key: 'M'}, {name: 'Production', key: 'P'}, {name: 'Research', key: 'R'}];
 ...
 // photoUrl: any;
  ...
  setPhotoPath () {   
   ...
   setDateOfBirth() {
    ...
    this.yearRange = `${new Date().getFullYear() - 10}:${new Date().getFullYear() - 7}`;
   }
   testfun1() {
    var a = [], b = [], prev;
  ...
  this.studentEnrollmentService.upload(formData).subscribe(
    res => {
        if (res.status === 'error') {
          ...
          this.messageService.add({key: 'custom', severity:'success', summary: 'Photo upload Successfully'});
         }
    },
 ...
 this.studentEnrollmentService.bookEventData(formData).subscribe(
  res => {
      if (res.status === 'error') {
        ...
        this.messageService.add({key: 'custom', severity:'success', summary: 'Event Booked Successfully'});
      }
  },
  error => this.error = error
);
...
cancelBooking() {
  this.studentEnrollmentService.cancelBooking(Number(this.bookingId)).subscribe(
    res => {
         this.messageService.add({key: 'custom', severity:'success', summary: 'Data Deleted Successfully'});
         this.showStudentEnrollForm = false;
         this.testBo= false;
         this.makeEmptyForm();
...
enrollStudent(eventData: any,capacity: any, gender:any, ageRange) {
    ...
    this.ageRange = ageRange
    if(capacity > 0 && gender == 'boy' ) {
     this.panelHeading = eventData.gameName +" "+ "Boys Under "+this.ageRange;
     this.gender = '1';
     this.setCapacity  = capacity;
    }
    if(capacity > 0 && gender == 'girl' ){
      this.panelHeading = eventData.gameName+" "+ "Girls Under "+this.ageRange;
      this.gender = '2';
      this.setCapacity = capacity;
    }
...
setSubGame(gameName,gameType,gameId,ageRange, gender,category) {
 // this.url = '';
  ...
  this.studentEnrollmentService.getSubGameData(this.gameId, ageRange, gender,'staffadmin').subscribe(
    response => {
      if(response!=="") {
        ...
        this.subGameResponseData.forEach(element => {
          ...
          } else if(element.gameType === 'Team' && element.eleven_girls > 0){
            this.mapSubGameTeam.push({
              'subGameId':element.id,
              'subGameName':element.subGameName,
          ...
          } else if(element.gameType === 'Team' && element.fourteen_boys > 0){
            this.mapSubGameTeam.push({
              'subGameId':element.id,
              'subGameName':element.subGameName,
          ...
          } else if(element.gameType === 'Team' && element.fourteen_girls > 0){
            this.mapSubGameTeam.push({
              'subGameId':element.id,
              'subGameName':element.subGameName,
          ...
          } else if(element.gameType === 'Team' && element.sixteen_boys > 0){
            this.mapSubGameTeam.push({
              'subGameId':element.id,
              'subGameName':element.subGameName,
          ...
          } else if(element.gameType === 'Team' && element.sixteen_girls > 0){
            this.mapSubGameTeam.push({
              'subGameId':element.id,
              'subGameName':element.subGameName,
          ...
          } else if(element.gameType === 'Team' && element.seventeen_boys > 0){
            this.mapSubGameTeam.push({
              'subGameId':element.id,
              'subGameName':element.subGameName,
          ...
          } else if(element.gameType === 'Team' && element.seventeen_girls > 0){
            this.mapSubGameTeam.push({
              'subGameId':element.id,
              'subGameName':element.subGameName,
          ...
          }  else if(element.gameType === 'Team' && element.ninteen_boys > 0){
            this.mapSubGameTeam.push({
              'subGameId':element.id,
              'subGameName':element.subGameName,
          ...
          } else if(element.gameType === 'Team' && element.ninteen_girls > 0){
            this.mapSubGameTeam.push({
              'subGameId':element.id,
              'subGameName':element.subGameName,
         ...
         if(element.eleven_boys > 0) {
            ...
            const gameIdAndName = element.id  +','+ element.subGameName  +','+ element.sixteen_boys +','+ element.gameType;
            this.subGameoptions.push({
              label: element.subGameName,
              value:gameIdAndName
            });
          }  else if(element.sixteen_girls > 0) {
            const gameIdAndName = element.id  +','+ element.subGameName  +','+ element.sixteen_girls +','+ element.gameType;
            this.subGameoptions.push({
              label: element.subGameName,
              value:gameIdAndName
            });
      ...
      if(this.subGameResponseData.length > 0) { 
          ...
          for(let i=0;i<this.subGameResponseData.length;i++) {
              ...
              if (category == 'eleven_boys' || category == 'eleven_girls' || category == 'fourteen_boys' || category == 'fourteen_girls' || category == 'sixteen_boys' || category == 'sixteen_girls' ||category == 'seventeen_boys' ||category == 'seventeen_girls' ||category == 'ninteen_boys' ||category == 'ninteen_girls') {
                  if ( (this.subGameResponseData[i]['eleven_boys'] === '0') || (this.subGameResponseData[i]['eleven_girls'] === '0') || (this.subGameResponseData[i]['fourteen_boys'] === '0') || (this.subGameResponseData[i]['fourteen_girls'] === '0') || (this.subGameResponseData[i]['sixteen_boys'] === '0') || (this.subGameResponseData[i]['sixteen_girls'] === '0') || (this.subGameResponseData[i]['seventeen_boys'] === '0') || (this.subGameResponseData[i]['seventeen_girls'] === '0') || (this.subGameResponseData[i]['ninteen_boys'] === '0') || (this.subGameResponseData[i]['ninteen_girls'] === '0')) {
                    this.subGameCount++;
                    console.log('imtrrew')
                  // break;
                  }  else  {
...
showError() {
  ...
  this.messageService.add({key: 'bc', severity:'error', summary: 'Error', detail: 'hahasfa asfafjasf afakjgagjhahasfa asfafjasf afakjgagjaghahasfa asfafjasf afakjgagjagag afasg Message Content'});
}
addMapSubGame() {
...
onSubmit() {
 ...
 // formData.append('profile', this.profileForm.get('profile').value, this.fullFilename);
  ...
  formData.append('admissionNo', this.studentEnroolForm.get('admissionNo').value);
  ...
  if(this.submitButtonLabel == "Submit") {
       if(this.setCapacity == this.studentListArray.length) {
        console.log('im true');
        this.messageService.add({key: 'custom', severity:'error', summary: 'No seat available for this game'});
      } else {
       this.studentEnrollmentService.saveStudentData(formData).subscribe(
         res => {
             if (res.status === 'error') {
               ...
               this.addClass = true;
               this.messageService.add({key: 'custom', severity:'success', summary: 'New Student Added Successfully'});
   ...
   if (this.submitButtonLabel == "Update") { 
      ...
      res => {
          if (res.status === 'error') {
            ...
            this.messageService.add({key: 'custom', severity:'success', summary: 'Student Data updated Successfully'});
            this.studentEnroolForm.reset();
            this.loadStudentData();
            this.selectedSubGame1 = '';
