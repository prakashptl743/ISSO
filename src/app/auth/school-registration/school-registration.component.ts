 ...
 setInitialForm() {
  ...
  initialForm() {
      this.schoolForm = this.fb.group({
        ...
        schoolEmail: ['', [Validators.required, Validators.email,Validators.pattern('^[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,4}$')]],
        schoolPassword: [''],
        schoolInfra: ['', Validators.required],
        schoolTelePhone: ['', [Validators.required, Validators.pattern('^[0-9]+$'),Validators.min(1000000000), Validators.max(999999999999)]],
  ...
  get f() { console.log(this.schoolForm.controls); return this.schoolForm.controls; }
initialEmailForm() {
...
onEmailVerify() {
  ...
  this.schoolService.verifyEmail(emailVal).subscribe((data) => {
    console.log(data);
 ...
 //if(schoolID == '') {
    ...
    res => {
        if (res.status === 'error') {
          ...
          this.showEmailErrorMessage = false;
          this.messageService.add({key: 'custom', severity:'success', summary: 'You have registered Successfully'});
